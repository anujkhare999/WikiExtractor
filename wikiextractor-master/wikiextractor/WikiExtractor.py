...
redirect_file = "/ssd/finalDump/Dump_temp/redirect.csv"
articles_c = 0
prev_time = 0
next_time = 0
ENCODING = "utf-8"
...
class OutputSplitter():
    ...
    def write(self, data):
        ...
        if self.compress:
            ...
            if articles_c % 1000 == 0:
                global prev_time
                global next_time
                next_time = time.time()
                cur_time = str(next_time-prev_time)
                prev_time=next_time
                logging.info("Saved %d pages in %s time", articles_c,cur_time)
            with codecs.open(pathDetails, "a", ENCODING) as articlesFH:
                articlesWriter = csv.writer(articlesFH, quoting=csv.QUOTE_MINIMAL)
                end_index = data.find(">")
                articlesWriter.writerow([ self.file.name,data[0:end_index+2]])
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    for line in input:
        ...
        if tag == 'page':
            ...
            end_point=line.rfind('"')
        elif tag == 'text':
            inText = True
            page.append(redirect_val)
            if m.lastindex == 4:  # open-close
                inText = False
        elif tag == '/text':
            if m.group(1):
            ...
            page = []
        if temp_flag:
            with codecs.open(redirect_file, "a", ENCODING) as articlesRedirect:
                articlesRedirect = csv.writer(articlesRedirect, quoting=csv.QUOTE_MINIMAL)
                articlesRedirect.writerow([line[start_point+2:end_point],title])
    input.close()
