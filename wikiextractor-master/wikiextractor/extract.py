...
import logging
import time
import codecs	#edited by Kapil
import csv
# ----------------------------------------------------------------------
mention_id = 1
#path to Store surfaceNames and articledetails- Kapil
pathArticles = "/ssd/finalDump/Dump_temp/src_dest_SN.csv"
pathDetails = "/ssd/finalDump/Dump_temp/artictDetails.csv"
pathContext = "/ssd/finalDump/Dump_temp/Context.csv"
ENCODING = "utf-8" 
'''
with codecs.open(pathArticles, "a", ENCODING) as articlesFH:
    articlesWriter = csv.writer(articlesFH, quoting=csv.QUOTE_MINIMAL)
    articlesWriter.writerow(["src_page_id","src", "dest", "surface_name","mention_id"])
with codecs.open(pathContext, "a", ENCODING) as contextFH:
    contextWriter = csv.writer(articlesFH, quoting=csv.QUOTE_MINIMAL)
    contextWriter.writerow(["mention_id","context"])
'''
...
def clean(extractor, text,cur_id, title, expand_templates=False, html_safe=True):
    """
    ...
    text = replaceInternalLinks(text,cur_id,title)
...
def replaceInternalLinks(text,cur_id,pageName): #Edited by Kapil 
    """
    ...
    for s, e in findBalanced(text, ['[['], [']]']):
        ...
        if end < context_end-150: 
            context_end = end + 150 
#         print(text[context_start:context_end])
        res += text[cur:s] + makeInternalLink(text[context_start:context_end],title,cur_id, label,pageName) + trail
        cur = end
    return res + text[cur:]

def saveLink(context,src,dest,cur_id,surface_name): #Added by Kapil

    with codecs.open(pathArticles, "a", ENCODING) as articlesFH:
        articlesWriter = csv.writer(articlesFH, quoting=csv.QUOTE_MINIMAL)
        articlesWriter.writerow([cur_id,src, dest, surface_name , context])


def makeInternalLink(context,title,cur_id, label,pageName): #Edited by Kapil 
    colon = title.find(':')
    ...
    if colon == 0:
        ...
        if colon2 > 1 and title[colon + 1:colon2] not in acceptedNamespaces:
            return ''
    if Extractor.keepLinks:
        saveLink (context,pageName,title,cur_id,label)
...
class Extractor():
    ...
    def clean_text(self, text,cur_id, mark_headers=False, expand_templates=False,
                   html_safe=True):
        """
        ...
        text = clean(self, text,cur_id,pagetitle, expand_templates=expand_templates,
                     html_safe=html_safe)  #Modified by Kapil
    ...
    def extract(self, out, html_safe=True):
        ...
        cur_id = self.id
        text = ''.join(self.page)
        text = self.clean_text(text,cur_id, html_safe=html_safe)
